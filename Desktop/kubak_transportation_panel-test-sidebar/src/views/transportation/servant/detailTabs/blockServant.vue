<template>
  <Splitter class="w-full grid py-6" style="min-width: 700px" layout="vertical">
    <SplitterPanel class="w-full grid">
      <div class="col-12 md:col-6 lg:col-5 mb-4">
    ...
    <SplitterPanel class="w-full" v-if="blockList && blockList.length > 0">
      <p class="mt-4 mx-2 text-md font-medium">
        {{ $t("transportation.users.block_list_message") }}
      </p>
      <div
        class="w-full grid mt-4"
        v-for="(block, index) in blockList"
        :key="index"
      >
        <div class="col-12 md:col-6 lg:col-4 mb-4" v-if="block.serviceType">
          <div class="p-inputgroup">
            <span class="p-inputgroup-addon">
              <ion-icon name="car-outline" class="text-xl"></ion-icon>
            </span>
            <!-- <InputText
              :value="block?.serviceType?.translations[0]?.title"
              :disabled="true"
            /> -->
          </div>
        </div>
        <div class="col-12 md:col-6 lg:col-4 mb-4">
          <div class="p-inputgroup">
            ...
            <InputText
              :value="
                moment(block.blockedAt).format('DD/MM/YYYY') +
                '  ' +
                moment(block.blockedAt).format('HH:MM:SS')
              "
              :disabled="true"
            />
          </div>
        ...
        <div class="mb-5 col-12 flex justify-content-end">
          <Button
            @click="updateBlock(block as BlockServantListResponse)"
...
import { ServantBlockService } from "@/backend/transport";
...
import {
  ...
  createMapBlockServantListResponseToBlockServantListResponseDto,
  createMapBlockServantListResponseToBlockServantRequest,
  createMapBlockServantRequestDtoToBlockServantRequest,
} from "./mapper";
...
const blockList = ref<BlockServantListResponseDto[]>([]);
const dialogRef: any = inject("dialogRef");
const route = useRoute();
const blockUserData = ref<BlockServantRequestDto>({
...
function getServantBlockList() {
  ServantBlockService.getServantBlocks({
    servantId: dialogRef.value?.data?.id,
  }).then((res) => {
    blockList.value =
      createMapBlockServantListResponseToBlockServantListResponseDto(res);
  });
}
onMounted(() => {
  getServantBlockList();
...
function blockUser() {
  ...
  ServantBlockService.postServantBlock({
...
function updateBlock(blockData: BlockServantListResponse) {
  ServantBlockService.updateServantBlock({
    requestBody:
      createMapBlockServantListResponseToBlockServantRequest(blockData),
  }).then(async () => {
    toast.changeToastState({
