          ...
          <div class="p-inputgroup">
            ...
            >
              <template #value="valueProp">
                {{
                  valueProp.value != undefined
                    ? $t(`transportation.reports.isCloser.${valueProp.value}`)
              ...
              <template #option="slotProps">
                <p>
                  {{
                    $t(`transportation.reports.isCloser.${slotProps.option}`)
                  }}
                </p>
              </template>
            </Dropdown>
...
import { TaskReportActionService, type RoleDto } from "@/backend/transport";

const dialogRef: any = inject("dialogRef");
const toast = toastStore();
const { t } = useI18n();
const isCloser = ref(undefined);
const isCloserOptions = ref([true, false]);
...
onMounted(() => {
  if (dialogRef.value.data) {
    selectedRole.value = dialogRef.value?.data?.reportProblemData?.role;
    for (const index in dialogRef.value.data.reportProblemData.translations) {
      reportProblemLanguages.value.push({
        language: {
          id: +index,
          title:
            dialogRef.value.data.reportProblemData.translations[index].language,
          disabled: true,
        },
        title: dialogRef.value.data.reportProblemData.translations[index].name!,
      });
    }
  }
});
...
async function addReportProblem() {
  ...
  await TaskReportActionService.updateTaskReportAction({
    id: dialogRef.value?.data?.reportProblemData?.id,
    requestBody: {
      isCloser: isCloser.value ?? false,
    ...
    dialogRef.value.options.emits.onUpdateReportProblem();
  });
