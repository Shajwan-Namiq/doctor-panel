  ...
  <div class="p-4">
    <Splitter layout="vertical">
            ...
            <div class="p-inputgroup">
              ...
              <span class="p-float-label">
                <InputText
                  ...
                  id="inputgroup"
                  type="text"
                  v-model="newServiceBaseType.title"
                />
      ...
      <SplitterPanel class="pt-4">
        ...
        <div
          class="w-8 mt-4"
          v-if="newServiceBaseType?.serviceBaseTypeLanguages?.length! > 0"
        >
          <div>
            <DataTable
              :value="newServiceBaseType?.serviceBaseTypeLanguages!"
...
import { Language, type CreateServiceTypeRequest } from "@/backend/transport";
...
const newServiceBaseType = ref<CreateServiceTypeRequest>({
  title: "",
...
const vs$ = useVuelidate(ruleService, newServiceBaseType.value);

function addNewTypeLanguage() {
  submittedTypeLanguage.value = true;
  if (!v$.value.$invalid) {
    newServiceBaseType.value?.serviceBaseTypeLanguages?.push({
...
function deleteBaseTypeLanguage(data) {
  newServiceBaseType.value?.serviceBaseTypeLanguages?.splice(data.index, 1);
}
...
function checkLanguage(language) {
  const index = newServiceBaseType?.value?.serviceBaseTypeLanguages?.findIndex(
...
function submitForm() {
  ...
  if (!vs$.value.$invalid) {
    serviceStore.createServiceBaseType(newServiceBaseType.value);
  }
}
</script>
