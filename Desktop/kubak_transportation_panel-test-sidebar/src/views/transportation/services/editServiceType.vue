...
const updateServiceTypeData = ref<CreateServiceTypeRequest>({
  icon: {
    bytes: "",
    fileName: "",
  },
...
onMounted(() => {
  serviceType.value = dialogRef.value.data?.serviceType;
  serviceTypeLanguages.value = serviceType.value?.translations?.map(
    (element) => {
      const result = {
        language: {
  ...
  updateServiceTypeData.value.translations = serviceType.value?.translations;
});
const onImageSelected = (imageFile: File) => {
  changeFileToBase64(imageFile).then((res) => {
    updateServiceTypeData.value.icon!.bytes = res;
    updateServiceTypeData.value.icon!.fileName = imageFile.name;
  });
};
...
async function addServiceTypeLanguage() {
  ...
  updateServiceTypeData.value.translations!.push({
...
async function editServiceType() {
  invalidData.value.language = !updateServiceTypeData.value.translations?.find(
  ...
  if (
    updateServiceTypeData.value.icon &&
    updateServiceTypeData.value.icon.bytes === ""
  ) {
    delete updateServiceTypeData.value.icon;
  }
  await serviceStore
    .updateServiceType(serviceType.value.id, updateServiceTypeData.value)
    .then(() => {
...
function deleteCountryLanguage(data) {
  ...
  updateServiceTypeData.value.translations!.splice(data.index, 1);
}
</script>
