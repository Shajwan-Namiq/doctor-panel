            ...
            <div class="grid p-4" style="width: 100% !important">
              ...
              <div class="col-12 md:col-6 lg:col-4 xl:col-3 mb-4">
                <SearchUserComponent
                  ...
                  "
                  :existingUser="discountCodeData.users"
...
import { createMapperForDiscountCodeResponse } from "./mapDiscountCodeResponse";
...
function getDiscountCodeDetail() {
  DiscountCodeService.getDiscountCodeDetails({
    id: +route.params.id,
  }).then((response: any) => {
    createMapperForDiscountCodeResponse(response).then((res) => {
      discountCodeData.value = res;
      ...
      if (discountCodeData.value.redeemBoundary) {
        const polygon = new L.Polygon(discountCodeData.value.redeemBoundary);
        showDeleteShape.value = true;
        drawnItems.addLayer(polygon);
        map.setView(discountCodeData.value.redeemBoundary[0], 12);
      }
...
function submitData() {
  ...
  // Update discountCodeData.services with selected service IDs
  discountCodeData.value.services = selectedServices.value.map((service) => ({
    id: service.id,
  }));
  ...
  if (!v$.value.$invalid) {
    DiscountCodeService.updateDiscountCode({
