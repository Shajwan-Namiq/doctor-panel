<template>
  ...
  >
    <div
      ...
      v-tooltip.bottom="
        $t(`transportation.tasks.routeDetail.routeType.${option.value}`)
      "
    >
...
export default {
  ...
  methods: {
    defaultTabIndexes() {
      const opts = DomHandler.find(this.$refs.container, ".p-button");
      const firstHighlight = DomHandler.findSingle(
        this.$refs.container,
        ".p-highlight"
      );
      ...
      for (let i = 0; i < opts.length; i++) {
        if (
          (DomHandler.hasClass(opts[i], "p-highlight") &&
            ObjectUtils.equals(opts[i], firstHighlight)) ||
          (firstHighlight === null && i == 0)
        ) {
          this.focusedIndex = i;
        }
      }
    ...
    onBlur(event, option) {
      if (
        event.target &&
        event.relatedTarget &&
        event.target.parentElement !== event.relatedTarget.parentElement
      ) {
        this.defaultTabIndexes();
      }
