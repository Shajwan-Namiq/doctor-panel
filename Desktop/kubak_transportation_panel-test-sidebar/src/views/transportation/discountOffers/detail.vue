      ...
      <Splitter layout="vertical" style="height: 100%" class="p-4">
        <div class="addDataComponentWrapper">
            ...
            <div class="grid p-4" style="width: 100% !important">
              ...
              <div class="col-12 md:col-6 lg:col-4 xl:col-3 mb-4">
                <SearchUserComponent
                  ...
                  "
                  :existingUser="discountOffersData.users"
                      ...
                      <div class="p-inputgroup">
                        ...
                        <span class="p-float-label">
                          <InputNumber
                            autofocus
                            id="inputgroup"
                            v-model="requirementData.minTaskCount"
                            :class="{
                              'p-invalid': rv$.value?.$invalid,
                            }"
                          />
                          <label for="inputgroup">{{
                            $t(
                      ...
                      <div class="p-inputgroup">
                        ...
                        <span class="p-float-label">
                          <InputNumber
                            autofocus
                            id="inputgroup"
                            v-model="requirementData.maxRedemptions"
                          />
                          <label for="inputgroup">{{
                            $t(
                      ...
                      <div class="p-inputgroup">
                        ...
                        <span class="p-float-label">
                          <Dropdown
                            ...
                            v-model="requirementData.taskCompletionTimeFrame"
                          >
                            <template #value="slotProps">
                              <div>
                                {{
          ...
          <SplitterPanel>
            ...
            <div class="w-8 m-4" v-if="steps.length > 0">
...
import { thisMoment } from "@/composables/customRequest";
...
import { createMapperForDiscountOffersResponse } from "./mapDiscountOffersResponse";
...
const requirementData = ref<{
  ...
  taskCompletionTimeFrame: TimeFrame.ANY_TIME,
...
const rv$ = useVuelidate(requirementRules, requirementData);
...
onMounted(() => {
  mapRender();
  getDiscountOffersDetail();
});

function getDiscountOffersDetail() {
  DiscountOffersService.getDiscountOffer({
    id: +route.params.id,
  }).then((response: any) => {
    createMapperForDiscountOffersResponse(response).then((res) => {
      discountOffersData.value = res;

      // return the number of the steps and give it to preview object
      steps.value = res.steps.map((step: any) => ({
        step: step.step,
        value: step.value,
        max: step.max,
      }));

      if (discountOffersData.value.boundary) {
        const polygon = new L.Polygon(discountOffersData.value.boundary);
        showDeleteShape.value = true;
        drawnItems.addLayer(polygon);
        map.setView(discountOffersData.value.boundary[0], 12);
      }

      // Update selectedServices based on serviceIds from the response
      if (selectedServices.value) {
        selectedServices.value = services?.value?.filter((service) =>
          response.serviceIds?.includes(parseInt(service?.id))
        );
      }

      // get data and give it to the requirement data
...
function handleResponse(response) {
  if (response && response.requirement) {
    requirementData.value = response.requirement;
  }
}
function submitData() {
  submittedForm.value = true;

  // Check if minTaskCount is required and not provided
  if (
    requirementData.value.maxRedemptions !== undefined &&
    (requirementData.value.minTaskCount === undefined ||
      requirementData.value.minTaskCount === null)
  ) {
    toast.changeToastState({
      severity: t("toasts.error_header"),
      summary: t("toasts.update_err_desc", {
        field: t("fields.minTaskCount"),
      }),
      type: "error",
    });
    return;
  ...
  // Assign requirementData to discountOffersData.requirement
  // Set to undefined if both minTaskCount and maxRedemptions are null or undefined
  discountOffersData.value.requirement =
    requirementData.value.minTaskCount != null ||
    requirementData.value.maxRedemptions != null
      ? requirementData.value
      : undefined;

  // Update discountOffersData.services with selected service IDs
  discountOffersData.value.services =
  ...
  DiscountOffersService.updateDiscountOffer({
    id: +route.params.id,
...
function createShape(e) {
  const layer = e.layer;
  discountOffersData.value.boundary = layer._latlngs[0];
