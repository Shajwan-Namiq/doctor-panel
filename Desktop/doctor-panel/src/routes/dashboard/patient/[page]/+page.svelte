<script lang="ts">
  ...
  onMount(() => {
    activeUrlStore.set("/dashboard/magazine/1");
  });
  ...
  async function fetchData() {
    let result = await getMagazineData(data.supabase, currentPage, pageSize);
  ...
  async function goToPage(page: number) {
    currentPage = page;
    await fetchData();
    goto(`/dashboard/magazine/${currentPage}`); // Update the URL
  }
    ...
    <div class="overflow-x-auto">
      <table class="table-auto w-full">
        ...
        <tbody class="text-sm divide-y divide-gray-200">
          {#each $magazineData as item, index (item.id)}
            <tr>
              ...
              <td class="p-2 whitespace-nowrap">
                <div class="text-lg text-right">
                  <div class="flex justify-start items-center gap-2">
                    <button
                      on:click={() => {
                        goto(`/dashboard/magazine/detail/${item.id}`);
                      }}
